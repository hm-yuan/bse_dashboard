...
当前项目处于“真实基础数据 + 标准演示表 + 页面语义层”的过渡阶段。市场定位顶部 KPI 和部分页面模型已优先使用 processed 基础表，但市场定位页仍有若干图表函数只读 raw Excel；公司画像中也有少量为了地图联动、公司展开详情和股价小图直接读取 raw Excel 的逻辑。上述过渡实现后续应迁移到 processed 标准表。
...
* `data/raw/global_capital_market_ipo_financing_2026h1.csv`
...
基础处理脚本会同步输出 `city_coordinates.csv`。如果当前批次公司基础表缺少经纬度字段，但已有 `data/processed/city_coordinates.csv` 存在，脚本会复用既有城市坐标并写入质量日志。
...
* `data/raw/北交所日度成交情况2020-2024.xlsx` 和 `data/raw/市场交易统计(Wind统计).xlsx`：用于市场定位页“北交所交易规模成长”图，支持日度、月度、年度口径和成交额、换手率、成交占全 A 股比重切换；当前由图表函数只读合并，后续应迁移到 processed 标准表。
* `data/raw/全球主要资本市场情况.xlsx`：用于市场定位页“全球主要资本市场对比”图，当前由图表函数只读读取；后续应迁移到 processed 标准表。
* `data/raw/global_capital_market_ipo_financing_2026h1.csv`：由用户提供图片整理而成，用于市场定位页“全球资本市场融资规模”图，统计口径为 2026 年上半年 IPO 募资。
...
缓存只用于地图和坐标渲染辅助，不作为唯一事实来源。城市经纬度的项目级事实来源为 `data/processed/city_coordinates.csv`。
...
`city_coordinates.csv` 由 `data/raw/上市公司基本情况.xlsx` 中的城市、经度、纬度字段拆分生成，作为后续城市坐标匹配的稳定 processed 表。

处理规则：

1. 当前批次原始公司表包含 `经度`、`纬度` 时，处理脚本按城市聚合生成或更新 `city_coordinates.csv`。
2. 同一城市存在多家公司时，`longitude`、`latitude` 使用该城市有效坐标的中位数，`company_count` 记录参与聚合的公司数量。
3. 城市匹配使用 `city_key`，会去除空格，并兼容“北京/北京市”等常见城市后缀差异。
4. 后续 `上市公司基本情况.xlsx` 不再包含 `经度`、`纬度` 时，处理脚本不得报错或回退读取 raw 坐标，而应复用既有 `city_coordinates.csv`，按城市为公司数据补齐坐标。
5. 公司画像地理分布优先消费 `dim_company` 中的坐标；如缺失，则通过 `dashboard_data$city_coordinates` 按城市匹配。
6. 如当前批次缺少经纬度且 `city_coordinates.csv` 也不存在，需在 `data_quality_log.csv` 记录 warning。
...
* `province`：城市所属省份，优先继承原始公司表省份字段。
* `city`：城市名称。
* `city_key`：标准化后的城市匹配键。
* `company_count`：该城市参与坐标聚合的公司数量。
* `longitude`：城市经度。
* `latitude`：城市纬度。
* `source_file`：来源文件名。
* `last_update_time`：处理完成时间。
...
`longitude`、`latitude` 应由 `city_coordinates.csv` 按城市匹配生成；后续更新原始公司表时不要求继续维护经纬度列。
