...
该入口优先使用 `market_position_company_detail.csv` 作为公司基础清单，并补齐后续画像页面需要的标准表。若基础清单缺失，则使用 `R/sample_data.R` 构造最小演示公司清单。新增表中除从 `market_position_company_detail.csv` 继承的公司代码、名称、板块、上市日期、行业、市值、PE 等字段外，其余字段均为演示生成字段，不应作为真实业务事实使用。
...
`market_position_kpi.csv` 当前服务于“市场定位画像”顶部 KPI，字段口径如下：
...
公司维表。`company_code`、`company_name`、`board`、`listing_date`、`industry` 继承自基础公司清单；`province`、`city`、`strategic_sector`、`is_bse`、`is_high_tech`、`is_specialized_new` 为演示生成字段。

字段：`company_code`、`company_name`、`board`、`listing_date`、`province`、`city`、`industry`、`strategic_sector`、`is_bse`、`is_high_tech`、`is_specialized_new`。
...
财务期间表，用于后续上市公司画像和市场质量画像。全部财务字段为演示生成，生成时保持高科技和专精特新公司研发费用率相对更高，且营收增长与利润质量不完全同步。
...
融资事件表，用于后续市场发展画像。`company_code` 继承自公司清单；融资类型、金额、用途和状态为演示生成，金额与公司市值保持一定相关性。
...
上市审核储备表，用于后续市场发展画像。该表为候选企业演示数据，不对应当前已上市公司清单。
...
* 上市公司画像数据处理：公司经营、盈利、研发、融资用途等表。
* 市场发展画像数据处理：上市、融资、交易活跃、产品生态等事实表。
